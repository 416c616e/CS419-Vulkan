...
#define APP_SHORT_NAME		"Project 5"
#define APP_LONG_NAME		"Vulkan Course"
...
bool				LightingOn;			// toggle lighting in the application
bool				BufferMode;
...
VkResult			Init05MyIndexDataBuffer( IN VkDeviceSize size, OUT MyBuffer * pMyBuffer );
...
InitGraphics( )
{
	...
	Init12SpirvShader( "lighting-vert.spv", &ShaderModuleVertex );
	Init12SpirvShader( "lighting-frag.spv", &ShaderModuleFragment );
...
Init01Instance( )
{
	...
	VkApplicationInfo vai;
		...
		vai.pNext = nullptr;
		vai.pApplicationName = "Vulkan Course";
	...
	VkInstanceCreateInfo vici;
		...
		vici.enabledLayerCount = 0;//sizeof(instanceLayers) / sizeof(char *);
...
Init05MyVertexDataBuffer( IN VkDeviceSize size, OUT MyBuffer * pMyBuffer )
{
	...
	REPORT( "Init05MyVertexDataBuffer" );
	return result;
}
...
Init14GraphicsVertexFragmentPipeline( VkShaderModule vertexShader, VkShaderModule fragmentShader, VkPrimitiveTopology topology, OUT VkPipeline *pGraphicsPipeline  )
{
	...
	VkPipelineLayoutCreateInfo				vplci;
		...
		vplci.pPushConstantRanges = (VkPushConstantRange *)vpcr;
...
RenderScene( )
{
	...
	VkBuffer vertexBuffers[1] = { MyJustVertexDataBuffer.buffer };
	VkBuffer indexBuffers = { MyIndexDataBuffer.buffer };
	...
	if (BufferMode == false) {
		...
		vkCmdPushConstants(CommandBuffers[nextImageIndex], GraphicsPipelineLayout, VK_PIPELINE_STAGE_VERTEX_SHADER_BIT, 0, sizeof(struct arm), &Arm1);
		vkCmdDraw(CommandBuffers[nextImageIndex], vertexCount, instanceCount, firstVertex, firstInstance);

		vkCmdPushConstants(CommandBuffers[nextImageIndex], GraphicsPipelineLayout, VK_PIPELINE_STAGE_VERTEX_SHADER_BIT, 0, sizeof(struct arm), &Arm2);
		vkCmdDraw(CommandBuffers[nextImageIndex], vertexCount, instanceCount, firstVertex, firstInstance);

		vkCmdPushConstants(CommandBuffers[nextImageIndex], GraphicsPipelineLayout, VK_PIPELINE_STAGE_VERTEX_SHADER_BIT, 0, sizeof(struct arm), &Arm3);
		vkCmdDraw(CommandBuffers[nextImageIndex], vertexCount, instanceCount, firstVertex, firstInstance);

		result = VK_SUCCESS;
		//REPORT("vkCmdDraw");
	} else {
		vkCmdBindVertexBuffers(CommandBuffers[nextImageIndex], 0, 1, vertexBuffers, offsets);		// 0, 1 = firstBinding, bindingCount
		result = VK_SUCCESS;
		//REPORT("vkCmdBindVertexBuffers");

		vkCmdBindIndexBuffer(CommandBuffers[nextImageIndex], indexBuffers, 0, VK_INDEX_TYPE_UINT32);
...
Reset( )
{
	...
	Misc.uTime = 0.;
	Misc.uMode = Mode;
	Misc.uLightOn = LightingOn;
    
    // arms
    Arm1.armMatrix = glm::mat4( );
    Arm1.armColor  = glm::vec3(0.0f,1.0f,0.0f);
    Arm1.armScale  = 8;

    Arm2.armMatrix = glm::mat4( );
    Arm2.armColor  = glm::vec3(1.0f,0.0f,0.0f);
    Arm2.armScale  = 4;

    Arm3.armMatrix = glm::mat4( );
    Arm3.armColor  = glm::vec3(0.0f,0.0f,1.0f);
    Arm3.armScale  = 2;
}
...
UpdateScene( )
{
	...
	// arms
	Arm1.armScale = 4 + (glm::sin(((int)(Misc.uTime * 100) % (int)(200 * M_PI)) / 100.0f) * 2.0f);
	Arm2.armScale = 2 + (glm::sin(((int)(Misc.uTime * 100) % (int)(200 * M_PI)) / 100.0f) * 1.0f);
	Arm3.armScale = 1 + (glm::sin(((int)(Misc.uTime * 100) % (int)(200 * M_PI)) / 100.0f) * 0.5f);
	
	Arm1.armMatrix = glm::mat4();
	Arm1.armMatrix = glm::rotate(Arm1.armMatrix, Misc.uTime, glm::vec3(0,0,1) );

	Arm2.armMatrix = glm::translate( Arm1.armMatrix, glm::vec3(Arm1.armScale*2.0f,0,0) );
	Arm2.armMatrix = glm::rotate(Arm2.armMatrix, Misc.uTime * 2.0f, glm::vec3(0, 0, 1));

	Arm3.armMatrix = glm::translate(Arm2.armMatrix, glm::vec3(Arm2.armScale*2.0f, 0, 0));
	Arm3.armMatrix = glm::rotate(Arm3.armMatrix, Misc.uTime * 4.0f, glm::vec3(0, 0, 1));
...
InitGLFW( )
{
	...
	MainWindow = glfwCreateWindow( Width, Height, APP_SHORT_NAME, NULL, NULL );
	...
	if( action == GLFW_PRESS )
	{
		switch( key )
		{
			...
			case 'L':
				...
				fprintf( FpDebug, "Lighting Toggle\n");
				fflush(FpDebug);
				break;
			...
			case 'B':
				...
				fprintf(FpDebug, "Buffer mode switch\n");
				fflush(FpDebug);
				break;
